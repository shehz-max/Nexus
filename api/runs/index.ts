...
export async function GET(request: NextRequest) {
  ...
  if (status) filteredRuns = filteredRuns.filter(r => r.status === status);
  if (workflowId) filteredRuns = filteredRuns.filter(r => r.workflowId === workflowId);
  ...
  return NextResponse.json({
    ...
    data: {
      runs: paginatedRuns,
      pagination: { page, limit, total: filteredRuns.length, totalPages: Math.ceil(filteredRuns.length / limit) }
    }
  });
}